import snippets from "@/data/snippets.json";
import Link from "next/link";

export default function SnippetsPage() {
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">片段 (Snippets)</h1>
      <p className="text-gray-600 dark:text-gray-400 mb-6">
        Total snippets: {snippets.length}
      </p>

      <ul className="space-y-4">
        {snippets.map((snippet) => (
          <li
            key={snippet.id}
            className="border p-4 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          >
            <Link href={`/snippets/${snippet.slug}`}>
              <h2 className="text-xl font-semibold">{snippet.title}</h2>
              <p className="text-gray-600 dark:text-gray-300">
                Language: {snippet.language}
              </p>
              <pre className="text-sm text-gray-500 dark:text-gray-400 mt-2 truncate">
                {snippet.code.substring(0, 80)}...
              </pre>
            </Link>
          </li>
        ))}
      </ul>
    </div>
  );
}
